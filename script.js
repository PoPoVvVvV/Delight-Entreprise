...
const validCredentials = [
    { username: 'Delight', password: 'Delight2104' }
...
function initializeSalesBook() {
    ...
    // const salespersonNameElement = document.getElementById('salesperson-name'); // Old way (getting the th)
    ...
    if (validateButton) {
        validateButton.addEventListener('click', () => {
            // const salesperson = salespersonNameElement.textContent.trim(); // Old way
            const salesperson = employeeSelect.value; // <<< NEW: Get the selected value from the dropdown

             // Check if an employee is selected
            if (!salesperson) {
                alert("Veuillez sélectionner un employé.");
                return; // Stop the process if no employee is selected
            }
            ...
            // Créer un objet avec les données de résumé à envoyer à Google Sheets
            const googleSheetsDataToSend = {
                Date: saleDate,
                Heure: saleTime,
                Vendeur: salesperson, // Now uses the selected value from the dropdown
                Chiffre_Affaire: totalAmount,
                Marge_Totale: totalProfit // Inclure la marge totale pour Google Sheets
            };

            console.log('Données sommaires à envoyer à Google Sheets :', googleSheetsDataToSend);
            ...
            if (googleSheetsUrl === 'YOUR_APPS_SCRIPT_WEB_APP_URL') {
                 ...
                 // Continue to attempt Discord send if configured, or return if not.
                 ...
                 fetch(googleSheetsUrl, {
                     ...
                     body: JSON.stringify(googleSheetsDataToSend) // Envoyez l'objet JSON résumé
                 })
                 .then(response => {
                      ...
                      updateDateTime();
                      // Reset the employee selection as well
            ...
            if (discordWebhookUrl === 'YOUR_DISCORD_WEBHOOK_URL') {
                ...
                const discordPayload = {
                    ...
                    content: `**Nouvelle Vente Validée**:\n` +
                             ...
                             `> **Heure:** ${saleTime}\n` +
                             `> **Vendeur:** ${salesperson}\n` +
                             `> **Chiffre d'Affaire:** $${totalAmount.toFixed(2)}\n` +
                             `> **Marge Totale:** $${totalProfit.toFixed(2)}`,
                ...
                .then(response => {
                    if (response.ok) {
                        ...
                        response.text().then(text => console.error('Corps de l\'erreur Discord :', text));
            ...
            // The Discord fetch runs in parallel or slightly after.
